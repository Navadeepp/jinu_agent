...
@app.get("/status")
async def status():
    return {"status": "running", "service": "J.I.N.U Backend"}

@sio.event
async def connect(sid, environ):
    print(f"Client connected: {sid}")
    await sio.emit('status', {'msg': 'Connected to J.I.N.U Backend'}, room=sid)
...
async def start_audio(sid, data=None):
    ...
    if audio_loop:
        if loop_task and (loop_task.done() or loop_task.cancelled()):
             ...
             await sio.emit('status', {'msg': 'J.I.N.U Already Running'})
             return
    ...
    try:
        ...
        print("Emitting 'J.I.N.U Started'")
        await sio.emit('status', {'msg': 'J.I.N.U Started'})
        ...
        print(f"CRITICAL ERROR STARTING JINU: {e}")
        import traceback
        traceback.print_exc()
...
async def stop_audio(sid):
    ...
    if audio_loop:
        ...
        audio_loop = None
        await sio.emit('status', {'msg': 'J.I.N.U Stopped'})
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to J.I.N.U to enable printer discovery"})
            return
